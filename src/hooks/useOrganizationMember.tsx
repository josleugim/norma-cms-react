...
import type { OrganizationMemberListResponse } from "../types/organization-member";
import type { Organization } from "../types/organization";
import { getOrganizations } from "../api/organization";
import { getOrganizationMembers } from "../api/organization-member";
import type { DataTableStateEvent } from "primereact/datatable";

const DEFAULT_PAGE_SIZE = 10;
const ROWS_PER_PAGE_OPTIONS = [10, 20, 50];

const useOrganizationMember = () => {
    const [first, setFirst] = useState(0);
    const [rows, setRows] = useState(10);
    const [organizationId, setOrganizationId] = useState<number | undefined>(undefined);
    const [data, setData] = useState<OrganizationMemberListResponse>({
        items: [],
        page: 1,
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            try {
                ...
                if (!cancelled) {
                    setError(err instanceof Error ? err.message : 'Error al cargar los miembros de la organización');
                }
            } finally {
                if (!cancelled) {
                    setIsLoading(false);
