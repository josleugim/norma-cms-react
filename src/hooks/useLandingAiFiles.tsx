...
import { getLandingAiDownloadUrl, getLandingAiFiles } from '../api/landing-ai';
...
const useLandingAiFiles = () => {
    const [files, setFiles] = useState<LandingAiFile[]>([]);
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            try {
                const response = await getLandingAiFiles();
                if (!cancelled) {
                    setFiles(response);
                }
            } catch (err) {
    ...
    const downloadFile = useCallback(async (file: LandingAiFile) => {
        setDownloadingKey(file.key);

        try {
            const { url } = await getLandingAiDownloadUrl(file.key);
...
export default useLandingAiFiles;
